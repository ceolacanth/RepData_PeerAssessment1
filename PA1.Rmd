...
The mean total of steps taken per day is: `r mean_steps`
...
The interval with the highest average steps is `r interval_with_max_steps_average`
...
Missing Data Values: `r missing_values`
...
Mean steps per day (imputed): `r round(mean(imputed_steps_per_day$x), digits = 0)`  
Mean steps per day (original): `r round(mean(steps_per_day$x), digits = 0)`  

Median steps per day (imputed): `r round(median(imputed_steps_per_day$x), digits = 0)`  
Median steps per day (original): `r round(median(steps_per_day$x), digits = 0)`  
